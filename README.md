...
30 c на профиль; 0 ошибок из 110 тыс. запросов финального прогона).

Что сделано между «до» и «после» (оптимизировался только main-api):

1. **4 воркера uvicorn** вместо одного (`--workers 4` в entrypoint) — CPU-ядра
   для сериализации и ORM; пул БД пересчитан **на процесс**:
   4 × (pool_size 5 + max_overflow 10) = 60 ≤ `max_connections` Postgres (100).
2. **Kafka `send()` вместо `send_and_wait`** — POST не ждёт подтверждения
   брокера (~20 мс); размен на гарантию доставки, честное решение дилеммы —
   transactional outbox, отложен.
3. **orjson** (`default_response_class=ORJSONResponse`) — быстрая сериализация
   JSON-ответов.

| Профиль | RPS до → после | p95 до → после | Ошибки |
|---------|----------------|----------------|--------|
| auth: `GET /me` (JWT + чтение БД, не трогали) | 1 231 → 1 307 | 32 → 29 мс | 0 |
| main-api: `GET /entries/` | 413 → **937** | 37 → **25 мс** | 0 |
| main-api: `POST /entries/` (INSERT + Kafka) | 139 → **310** | 64 → **29 мс** | 0 |
| сквозной (auth + main-api + search, 5 операций) | 693 → **1 149** | 61 → **45 мс** | 0 |
| search: gRPC `Search` напрямую (не трогали) | 8 237 → 7 419 | 2.9 → 3.7 мс | 0 |

Что важно понимать про эти цифры:

- **Прирост main-api ×2.3** дали в основном воркеры; auth и search не менялись —
  их дельты показывают разброс между прогонами (±5–10%), на его фоне и надо
  читать остальные цифры.
- **SQL-поиск `?q=` замедлился с ростом базы**: нагрузочные прогоны накопили
  ~60 тыс. записей, и ILIKE-скан по ним подорожал (p50 27 → 40 мс). Это
  ожидаемая деградация неиндексированного поиска — и живой аргумент в пользу
  семантического search-сервиса или GIN-индекса (`pg_trgm`) на дальних фазах.
